...
export default function HeaderProfile() {
  return (
    <section id="headerprofile" className="relative mb-8 bg-gray-800 bg-opacity-50 p-4 md:p-6 rounded-lg backdrop-blur-sm shadow-lg overflow-hidden">
      {/* Background effect */}
      ...
      <div className="relative z-10">
        <div className="flex flex-col md:flex-row items-center">
          ...
          <div className="flex-1 text-center md:text-left md:ml-6">
            ...
            >
              <p className="leading-relaxed text-sm md:text-base px-4 md:px-0">
        ...
        <motion.div
          ...
          animate={{ opacity: 1 }}
          transition={{ duration: 0.5, delay: 0.4 }}
          className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 mt-6"
        >
          {[
            ...
            <motion.div 
              key={index}
              className={`flex flex-col items-center p-3 md:p-4 bg-gradient-to-br ${item.color} rounded-lg hover:shadow-lg transition-all duration-300`}
              whileHover={{ scale: 1.05 }}
            >
              <item.icon className="w-6 h-6 md:w-8 md:h-8 text-white mb-2 opacity-75" />
              <span className="text-white text-xs md:text-sm font-medium text-center">{item.text}</span>
            </motion.div>
          ))}
        </motion.div>
      </div>
